...
function updateCalendar(e) {
    ...
    updateTitle();
    generateCalendar();
    for (let i = 0; i < tableauReservationsEvenements.length; i++) {
        const map = new Map(Object.entries(tableauReservationsEvenements[i]));  
...
function ajouterReservation(reservation) {
    ...
    var endOfWeek = moment(currentDate).endOf('isoWeek');
        endOfWeek = endOfWeek.set({'hour' : 22, 'minute' : 0, 'second' : 0}).toDate();
    ...
    do {
        ...
        if (dateDebut.getDate() < dateFin.getDate()) {
            dateFinCurrentDay =  moment(dateFin).set({'date' : dateDebut.getDate(), 'hour' : 22, 'minute' : 0}).toDate();
        }
        ...
        tableTd.innerText = reservation.get("nomReservation").charAt(0).toUpperCase() + reservation.get("nomReservation").slice(1);
