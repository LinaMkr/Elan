...
include_once(__DIR__."/../model/equipement.class.php");
...
if($utilisateur!=null && $admin){ // Si quelqu'un de connecté et admin
  ...
  if(isset($_GET['action']) && isset($_GET['code']) && $_GET['action']=='supprimer'){
    ...
    if($code==1){ // $code = 1 : code de réussite
      ...
      $view->assign('message',"Vous avez bien supprimé l'équipement.");

    }else{ // $code = -2 : code d'erreur inconnue

      // Affichage d'un message d'erreur
      $view->assign('erreur',"Vous ne pouvez pas supprimer l'équipement : erreur inconnue.");
    ...
    if($code==1){ // $code = 1 : code de réussite
      ...
      // Affichage d'un message de réussite
      $view->assign('message',"Vous avez bien créé l'équipement.");

    }else if($code==-1){ // $code = -1 : code d'erreur parce que nom déjà assigné

      // Affichage d'un message d'erreur
      $view->assign('erreur',"Nom de l'équipement déjà existant.");

    }else{ // $code = -2 : code d'erreur inconnue

      // Affichage d'un message d'erreur
      $view->assign('erreur',"Vous ne pouvez pas créer l'équipement : erreur inconnue.");
    }
      ...
      if($code){ // $code = true : code de réussite
        ...
        // Affichage d'un message de réussite
        $view->assign('message',"Vous avez bien assigné l'équipement à la salle.");

      }else{ // $code = false : code d'erreur inconnue

        // Affichage d'un message d'erreur
        $view->assign('erreur',"Vous ne pouvez pas assigner l'équipement à cette salle.");

      }
  }


  // Récupération de tous les equipements
  $equipements = $dao->getEquipements();

  // Récupération de tous les lieux
  $salles = $dao->getLieux();

  // Assignation des objets Equipements et Lieux à la page
  $view->assign('equipements',$equipements);
  $view->assign('salles',$salles);
  ...
  $view->display("adminGestionEquipement.view.php");
