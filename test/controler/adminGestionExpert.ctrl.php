...
if($utilisateur!=null && $admin){ // Si quelqu'un de connecté et admin
  ...
  // Affcetation des experts et des typeReservation
  $view->assign('experts',$dao->getExperts());
  $view->assign('typeResas',$dao->getTypesResa());

  // AFFICHAGE D'UN MESSAGE SI UNE ACTION A ETE EFFECTUE
  ...
  if(isset($_GET['action']) && isset($_GET['code']) && $_GET['action']=='supprimer'){
    ...
    if($code==1){ // $code = 1 : code de réussite
      ...
      $view->assign('message',"Vous avez bien supprimé l'expert.");

    }else{ // $code = -2 : code d'erreur inconnue

      // Affichage d'un message d'erreur
      $view->assign('erreur',"Vous ne pouvez pas supprimer l'expert : erreur inconnue.");
    ...
    if($code==1){ // $code = 1 : code de réussite
      ...
      // Affichage d'un message de réussite
      $view->assign('message',"Vous avez bien créé l'expert.");

    }else if($code==-1){ // $code = -1 : code d'erreur parce que nom et prenom déjà assigné

      // Affichage d'un message d'erreur
      $view->assign('erreur',"Nom et prénom de l'expert déjà existant.");

    }else{ // $code = -2 : code d'erreur inconnue

      // Affichage d'un message d'erreur
      $view->assign('erreur',"Vous ne pouvez pas créer l'expert : erreur inconnue.");
    }
    ...
    if($code){ // $code = true : code de réussite
      ...
      // Affichage d'un message de réussite
      $view->assign('message',"Vous avez bien assigné l'expert à un type de réservation.");

    }else{ // $code = false : code d'erreur inconnue

      // Affichage d'un message d'erreur
      $view->assign('erreur',"Vous ne pouvez pas assigner l'expert à ce type de réservation.");
  ...
  $view->display("adminGestionExpert.view.php");
